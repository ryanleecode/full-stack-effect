  ...
  cjsFiles.map(async (file) => {
    ...
    visit(code, {
      visitImportDeclaration(path) {
        path.value.source.value = fixImportPath(
      ...
      visitCallExpression(path) {
        if (
          ...
          path.value.arguments[0].value = fixImportPath(
      ...
      visitTSImportType(path) {
        path.value.argument.value = resolvePathAlias(
          path.value.argument.value,
          file,
        );
        this.traverse(path);
      },
...
await Promise.all(
  esmFiles.map(async (file) => {
    ...
    visit(code, {
      ...
      visitTSImportType(path) {
        path.value.argument.value = fixImportPath(
          path.value.argument.value,
          file,
          '.js',
        );
